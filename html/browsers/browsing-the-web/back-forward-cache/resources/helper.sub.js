...
async function getBFCachedStatus(target) {
  const [loadCount, isPageshowFired] =
    await target.execute_script(() => [window.loadCount, window.isPageshowFired]);
...
// 1. The page A is opened by `openFunc(url)`.
// 2. `func` is executed on page A (if not null).
// 3. The window is navigated to page B on `targetOrigin`.
// 4. The window is back navigated to page A (expecting BFCached).
//
// Events `events` (an array of strings) are observed on page A and
// `expectedEvents` (an array of strings) is expected to be recorded.
// See `event-recorder.js` for event recording.
function runEventTest(
    openFunc, func, targetOrigin, events, expectedEvents, description) {
  ...
  promise_test(async t => {
    ...
    const urlA = executorPath + pageA.context_id +
                 '&events=' + events.join(',');
    const urlB = targetOrigin + executorPath + pageB.context_id;

    window.open(urlA, '_blank', 'noopener');

    await pageA.execute_script(waitForPageShow);
    if (func) {
      await pageA.execute_script(func);
    }
