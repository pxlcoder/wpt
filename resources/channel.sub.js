(function() {
    ...
    self.ctx_channel = async function() {
        ...
        let channel = new TestDriverCommandRecvChannel(new RecvChannel(uuid));
    ...
    class TestDriverCommandRecvChannel {
        ...
        async connect() {
            console.log(`TestDriverCommandRecvChannel connect ${this.uuid}`);
            await this.channel.connect();
        }

        async close() {
            console.log(`TestDriverCommandRecvChannel close  ${this.uuid}`);
...
Promise.resolve(result).then(callback);`;
    ...
    class TestDriverRemote {
        constructor(dest) {
            ...
            console.log("TestDriverRemote", this.sendChannel);
        ...
        async connect() {
            ...
            console.log("TestDriverRemote.connect", this.uuid);
        ...
        async sendMessage(command, params, hasResp=true) {
            console.log("TestDriverRemote.sendMessage", this.uuid, command, params);
            if (!this.connected) {
                await this.connect();
            }
        ...
        postMessage(msg) {
            ...
            return this.sendMessage("postMessage", {msg}, false);
        }
